...
def db(tmp_path):
    store.reset_for_tests(str(tmp_path / "gh.db"))
...
def test_klon_url_traegt_den_token():
    settings.set_many({"github_token": "ghp_geheim"})
    url = gh_mod.GitHubClient().clone_url("owner/repo")
    assert url == "https://x-access-token:ghp_geheim@github.com/owner/repo.git"


def test_klon_url_wird_fuer_die_ausgabe_geschwaerzt():
    settings.set_many({"github_token": "ghp_geheim"})
    url = gh_mod.GitHubClient().clone_url("owner/repo")
